...
public class StudentView {
	...
	// 인덱스를 먼저 입력 받고 해당 인덱스에 학생이 존재하면 Html, CSs, JS, Java 점수를 입력받아
	// 해당 학생에게 세팅(set)
	...
	private void updateStudent() {
		...
		if(service.selectIndex(index) == null) {
			...
			// HTML,CSS JS, JAVA 점수 입력 받고 바로 std에 세팅
		
		System.out.print("HTML CSS JS Java 순서대로 점수 입력 : ");
	 ...
	 * 2) 있을 경우 service.selectScore(인덱스)를 호출한 후 과목(n)... 합계 : m 평균 : x 형태의 문자열로 반환 */
	private void selectScore() {
		System.out.print("점수 조회할 학생의 인덱스 번호를 입력 : ");
		int index = sc.nextInt();
		Student std = service.selectIndex(index);
		if(service.selectIndex(index) == null) {
			System.out.println("해당 인덱스에는 학생이 존재하지 않습니다.");
			return;
		}
		int sum = std.getHtml() + std.getCss() + std.getJs() + std.getCss();
		double avg = (double)sum / 4;
		System.out.printf("%s 학생의 점수 : HTML(%d) CSS(%d) JS(%d) Java(%d)\n 합계 : %d\n 평균 : %.1f",std.getName(),std.getHtml(),std.getCss(),std.getJs(),std.getJava(),sum,avg);
